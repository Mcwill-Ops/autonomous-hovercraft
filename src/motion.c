...
void turn_by_yaw(float delta_deg)
{
    ...
    if (delta_deg > 0)
        set_servo_angle(ANG_LEFT_DEG);
    else
        set_servo_angle(ANG_RIGHT_DEG);
    ...
    while (1)
    {
        ...
        if (delta_deg > 0 && y >= delta_deg - 5.0f)
            break;
        if (delta_deg < 0 && y <= delta_deg + 5.0f)
            break;
...
void advance_then_turn(bool goLeft, float advance_cm)
{
    ...
    if (targetReading < MIN_FRONT_CM)
        targetReading = MIN_FRONT_CM;
